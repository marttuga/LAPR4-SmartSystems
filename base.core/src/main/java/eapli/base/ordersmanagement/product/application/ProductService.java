...
/*    public Product createProduct(UniqueInternalCode uniqueInternalCode, ShortDescription shortDescription, ExtendedDescription extendedDescription, TechnicalDescription technicalDescription, Brand brand, Reference reference, ProductionCode productionCode, Weight weight , Barcode barcode, ProductPriceDetail priceDetail, Category categoryCode, Dimension dimension, Aisle aisle, Row row, Shelf shelf) {

        final ProductBuilder productBuilder = new ProductBuilder();
        Product product;
        product = productBuilder.build();
        productBuilder.withUniqueInternalCode(uniqueInternalCode).withShortDescription(shortDescription).withExtendedDescription(extendedDescription).withTechnicalDescription(technicalDescription).withBrand(brand).withReference(reference).withProductionCode(productionCode).withWeight(weight).withBarcode(barcode).withPriceDetail(priceDetail).withCategory(categoryCode).withDimension(dimension).withAisle(aisle).withRow(row).withShelf(shelf) ;
